    ...
    if request.method == "POST":
        if "lockRoom" in request.POST:
            ...
            if check:
                ...
                messages.error(request, "Номер занят на эту дату")
...
def current_room_services(request):
    ...
    if curBooking is not None:
        ...
        context = {
            "role": role,
            "error": "Вы еще не забронировали номер"
        }
    ...
    if request.method == "POST":
        if "decline" in request.POST or "approve" in request.POST:
            ...
            def send(request, receiver, text, subject):
                ...
                messages.success(
                    request, 'Отзыв был отправлен успешно')
            ...
            def send_mail_refund_approved(request, receiver):
                ...
                text = """
                    Дорогой {guestName},
                    Ваш запрос о возврате средств был принят.
                """
            ...
            def send_mail_refund_declined(request, receiver):
                ...
                text = """
                    Дорогой {guestName},
                    Ваш запрос о возврате средств был отменен.
                """
                email_text = text.format(
                    guestName=receiver.user.first_name + " " + receiver.user.last_name)
    ...
    if request.method == "POST":
        if "sendReq" in request.POST:
            ...
            if not temp:
                ...
                currentReq.save()
                messages.success(
                    request, "Просьба отправлена успешно")
            else:
                messages.error(
                    request, "Уже существует просьба о возврате")
